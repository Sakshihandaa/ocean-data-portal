...
export default function InstrumentCatalog({
  ...
  function removeChip(chip: { type: string; value: string }) {
    if (chip.type === "search") setQuery("");
    else if (chip.type === "location") setLocationFilter("");
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {pageItems.map((item) => (
                      <InstrumentCard
                        ...
                        instrument={item}
                        onClick={() => {
                          if (onSelectInstrument) onSelectInstrument(item);
                          else toast(item.name, { description: "Open details" });
                        }}
                      />
                    ))}
                  </div>
...
function FilterSections({
  ...
  selectedStatuses: Set<InstrumentStatus>;
  onToggleStatus: (s: InstrumentStatus) => void;
